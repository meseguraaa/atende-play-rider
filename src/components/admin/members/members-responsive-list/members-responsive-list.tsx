...
import {
    AdminMemberRow,
    AdminMemberRowMobile,
} from '@/components/admin/members/admin-member-row/admin-member-row';
...
type MembersResponsiveListProps = {
    rows: AdminMemberRowData[];
};
...
export function MembersResponsiveList({ rows }: MembersResponsiveListProps) {
        ...
        return (
            <section className="overflow-x-auto rounded-xl border border-border-primary bg-background-tertiary">
                <table className="w-full border-collapse text-sm">
                    ...
                    <tbody>
                        {rows.map((row) => (
                            <AdminMemberRow key={row.id} row={row} />
                        ))}
                    </tbody>
                </table>
            </section>
    ...
    return (
        <div className="space-y-2">
            {rows.map((row) => (
                <AdminMemberRowMobile key={row.id} row={row} />
            ))}
        </div>
    );
}
