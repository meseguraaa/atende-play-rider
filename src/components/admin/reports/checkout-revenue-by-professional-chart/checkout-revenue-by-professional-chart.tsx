...
type CheckoutRevenueByProfessionalChartItem = {
    name: string;
    value: number;
};

type CheckoutRevenueByProfessionalChartProps = {
    data: CheckoutRevenueByProfessionalChartItem[];
};

type ProfessionalTooltipPayload = {
    value: number | string | undefined;
    payload: CheckoutRevenueByProfessionalChartItem;
};

type ProfessionalTooltipProps = {
    active?: boolean;
    payload?: ProfessionalTooltipPayload[];
};
...
function CustomTooltip({ active, payload }: ProfessionalTooltipProps) {
...
export function CheckoutRevenueByProfessionalChart({
    data,
}: CheckoutRevenueByProfessionalChartProps) {
        ...
        <Card className="border-border-primary bg-background-secondary">
            <CardHeader>
                ...
                <CardDescription className="text-paragraph-small text-content-secondary">
                    Top profissionais por receita gerada no checkout no mês
                    selecionado.
                </CardDescription>
            </CardHeader>
                ...
                {chartData.length === 0 ? (
                    <div className="flex h-full items-center justify-center rounded-xl border border-dashed border-border-primary bg-background-secondary px-4 text-center">
                        <p className="text-paragraph-small text-content-secondary">
                            Nenhuma receita por profissional neste período.
                        </p>
                    </div>
                ) : (
