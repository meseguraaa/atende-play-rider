// src/components/admin/ride/new-ride-dialog/new-ride-dialog.tsx
'use client';
...
function getInitialEndsAt(startsAt: string) {
...
export default function NewRideDialog({ children }: Props) {
    ...
    const [endsAt, setEndsAt] = React.useState(
        getInitialEndsAt(initialStartsAt)
    );
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (!response.ok) {
                toast.error(data?.error ?? 'Não foi possível criar o rolê.');
                return;
            }

            toast.success('Rolê criado com sucesso!');
            setOpen(false);
            resetForm();
            router.refresh();
        } catch {
            toast.error('Erro ao criar rolê.');
        } finally {
            setSubmitting(false);
        }
    }
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            >
                <DialogHeader>
                    <DialogTitle size="modal">Novo rolê</DialogTitle>
                    <DialogDescription size="modal">
                        Preencha as informações principais para organizar o
                        próximo rolê do grupo.
                    </DialogDescription>
                </DialogHeader>
